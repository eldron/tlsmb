...
// simply read data from socket
SECStatus simple_read_data(PRFileDesc * sslSocket, DataBuffer * buffer){
	int    numBytes = 0;
...
setupSSLSocket(PRFileDesc *tcpSocket, int requestCert)
{
    ...
    if(secStatus != SECSuccess){
        exitErr("set ssl version failed\n");
    } else {
        fprintf(stderr, "set ssl version range succeeded\n");
    }
...
handle_connection(void *tcp_sock, int requestCert)
{
    ...
    secStatus = simple_read_data(sslSocket, &buffer);
	if (secStatus != SECSuccess) {
		goto cleanup;
	}
...
accept_connection(void *listener, int requestCert)
{
	...
	while (!stopping) {
		...
		fprintf(stderr, "About to call accept.\n");
