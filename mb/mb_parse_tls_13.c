...
gen_eckey_from_client_hello(struct MBTLSConnection * conn, 
    sslClientHelloType type, uint8_t * buffer, uint32_t length, PLArenaPool * arena)
{
    unsigned int i;
    ...
    for(cursor = PR_NEXT_LINK(&(ss->ephemeralKeyPairs)); cursor != &(ss->ephemeralKeyPairs); cursor = PR_NEXT_LINK(cursor)){
        ...
        if(keypair->group->name == ssl_grp_ec_secp256r1 ||
            ...
            SECItem A;
            A.len = public_key_len;
            A.data = PORT_ArenaAlloc(arena, public_key_len);
            // read form file
            unsigned int value;
            ...
            int gen_result = generate_ec_private_key_for_middlebox(ecParams, &A, NULL, &private_key_bytes, &public_key_bytes);
            if(gen_result == 0){
                fprintf(stderr, "generate ec private key for middlebox failed\n");
                return SECFailure;
            } else {
                fprintf(stderr, "generated private key is:\n");
                for(i = 0;i < private_key_len;i++){
                    fprintf(stderr, "%u ", private_key_bytes[i]);
                }
                fprintf(stderr, "\ngenerated public key is:\n");
                for(i = 0;i < public_key_len;i++){
                    fprintf(stderr, "%u ", public_key_bytes[i]);
                }
                fprintf(stderr, "\n\n");
            }
            ...
            rv = pk11_setAttributes(private_key_slot, private_key_handle,
                       &setTemplate, 1);
            if(rv != SECSuccess){
