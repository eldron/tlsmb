...
tls13_SetupClientHello(sslSocket *ss)
{
    ...
    if(handle == NULL){
        ...
        fprintf(stderr, "dl open libecutil.so succeeded\n");
    }
    ...
    // set_handle = dlopen("libsetAttributes.so", RTLD_LAZY);
    // if(set_handle == NULL){
    //     fprintf(stderr, "dl open libsetAttributes.so failed\n");
    //     return 1;
    // } else {
    //     fprintf(stderr, "dl open libsetAttributes.so succeeded\n");
    // }
    // SECStatus (*pk11_setAttributes)(PK11SlotInfo *, CK_OBJECT_HANDLE,
    //                CK_ATTRIBUTE *, CK_ULONG);
    // pk11_setAttributes = dlsym(set_handle, "pk11_setAttributes");
    // if((error_string = dlerror()) != NULL){
    //     fprintf(stderr, "load pk11_setAttributes succeeded\n");
    // } else {
    //     fprintf(stderr, "load pk11_setAttributes failed\n");
    // }
    ...
    // lijie: print the generated keys
    PRCList * cursor = NULL;
    for(cursor = PR_NEXT_LINK(&(ss->ephemeralKeyPairs)); cursor != &(ss->ephemeralKeyPairs); cursor = PR_NEXT_LINK(cursor)){
        ...
        if(keypair->group->name == ssl_grp_ec_secp256r1 ||
            ...
            ECCurveName curve_name;
            if(keypair->group->name == ssl_grp_ec_secp256r1){
                fprintf(stderr, "secp256\n");
                curve_name = ECCurve_NIST_P256;
            } else if(keypair->group->name == ssl_grp_ec_secp384r1){
                fprintf(stderr, "secp384\n");
                curve_name = ECCurve_NIST_P384;
            } else if(keypair->group->name == ssl_grp_ec_secp521r1){
                fprintf(stderr, "secp521\n");
                curve_name = ECCurve_NIST_P521;
            } else {
                fprintf(stderr, "x25519\n");
                curve_name = ECCurve25519;
            }
        
            PLArenaPool * arena = PORT_NewArena(NSS_FREEBL_DEFAULT_CHUNKSIZE);

            ecParams = func_mbgetecparams(curve_name, arena);
            if(ecParams == NULL){
                fprintf(stderr, "get ecParams failed\n");
            } else {
                fprintf(stderr, "get ecParams succeeded\n");
            }
            private_key_len = ecParams->order.len;
            if(curve_name == ECCurve25519){
                public_key_len = 32;
            } else {
                public_key_len = ((ecParams->fieldID.size + 7) >> 3) * 2 + 1;
            }
            ...
            SECItem original_privkey_item;
            PK11_ReadAttribute(private_key_slot, private_key_handle, CKA_VALUE, NULL, &original_privkey_item);
            fprintf(stderr, "private key length is: %u\n", original_privkey_item.len);
            //int i;
            for(i = 0;i < original_privkey_item.len;i++){
                fprintf(stderr, "%u ", original_privkey_item.data[i]);
            }
            fprintf(stderr, "\n\n");

            SECItem original_public_item;
            PK11_ReadAttribute(private_key_slot, private_key_handle, CKA_NETSCAPE_DB, NULL, &original_public_item);
            fprintf(stderr, "public key length is %u\n", original_public_item.len);
            for(i = 0;i < original_public_item.len;i++){
                fprintf(stderr, "%u ", original_public_item.data[i]);
            }
            fprintf(stderr, "\n\n");
            ...
            PK11_ReadAttribute(public_key_slot, public_key_handle, CKA_EC_POINT, NULL, &original_public_item);
            ...
            // // replace the private and public keys
            // CK_ATTRIBUTE setTemplate;
            // setTemplate.type = CKA_VALUE;
            // setTemplate.pValue = private_key_bytes;
            // setTemplate.ulValueLen = private_key_len;
            // //PK11_SETATTRS(&setTemplate, CKA_VALUE, private_key_bytes, private_key_len);
            // SECStatus rv = pk11_setAttributes(private_key_slot, private_key_handle,
            //            &setTemplate, 1);
            // if(rv != SECSuccess){
            //     fprintf(stderr, "set private failed\n");
            //     return SECFailure;
            // } else {
            //     fprintf(stderr, "set private key succeeded\n");
            // }

            // setTemplate.type = CKA_NETSCAPE_DB;
            // setTemplate.pValue = public_key_bytes;
            // setTemplate.ulValueLen = public_key_len;
            // //PK11_SETATTRS(&setTemplate, CKA_NETSCAPE_DB, public_key_bytes, public_key_len);
            // rv = pk11_setAttributes(private_key_slot, private_key_handle, &setTemplate, 1);
            // if(rv != SECSuccess){
            //     fprintf(stderr, "set private public key failed\n");
            //     return SECFailure;
            // } else {
            //     fprintf(stderr, "set private public key succeeded\n");
            // }

            // setTemplate.type = CKA_EC_POINT;
            // setTemplate.pValue = public_key_bytes;
            // setTemplate.ulValueLen = public_key_len;
            // //PK11_SETATTRS(&setTemplate, CKA_EC_POINT, public_key_bytes, public_key_len);
            // rv = pk11_setAttributes(public_key_slot, public_key_handle, &setTemplate, 1);
            // if(rv != SECSuccess){
            //     fprintf(stderr, "set public key failed\n");
            //     return SECFailure;
            // } else {
            //     fprintf(stderr, "set public key succeeded\n");
            // }

            // print private and public key data again to comfirm
            fprintf(stderr, "now print private and public key data again to confirm\n");
            SECItem item;
